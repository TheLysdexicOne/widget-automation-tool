...
from PyQt6.QtGui import QFont, QPixmap, QPainter, QPen, QBrush, QColor, QMouseEvent
...
class ScreenshotManagerDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.regions_button = QPushButton("View Regions")
        self.regions_button.clicked.connect(self._view_regions)
        self.regions_button.setEnabled(bool(self.primary_screenshot))
        ...
        button_layout.addWidget(self.make_primary_button)
        button_layout.addWidget(self.delete_button)
        button_layout.addWidget(self.regions_button)
        button_layout.addWidget(self.new_screenshot_button)
    ...
    def _make_primary(self):
        ...
        if new_primary != self.primary_screenshot:
            ...
            self.regions_button.setEnabled(True)
    ...
    def _view_regions(self):
        """View text regions on primary screenshot."""
        if not self.primary_screenshot:
            QMessageBox.information(self, "No Primary", "No primary screenshot selected.")
            return

        # Find primary screenshot file
        screenshot_path = None
        screenshots_dir = self.frames_manager.screenshots_dir
        for file_path in screenshots_dir.glob(f"*{self.primary_screenshot}*"):
            screenshot_path = file_path
            break
        ...
        # Show regions viewer dialog
        regions_dialog = RegionsViewerDialog(screenshot_path, self.frame_data, self)
        regions_dialog.exec()
