...
class MonitoringTab(BaseTab):
    ...
    def __init__(self, app, parent=None):
        ...
        if self.window_manager:
            self.window_manager.coordinates_updated.connect(
                self._on_coordinates_updated
            )
            self.logger.info("Connected to window_manager.coordinates_updated signal")
        else:
            self.logger.warning("No window_manager available")

        if self.mouse_tracker:
            self.mouse_tracker.position_changed.connect(self._on_mouse_moved)
            self.logger.info("Connected to mouse_tracker.position_changed signal")
        else:
            self.logger.warning("No mouse_tracker available")
    ...
    def _setup_ui(self):
        ...
        header.resizeSection(0, 150)  # Metric
        ...
        try:
            ...
            if self.window_manager:
                ...
                if self.process_monitoring_table.item(0, 1):
                    self.process_monitoring_table.item(0, 1).setText(
                        widget_info.get("status", "N/A")
                    )
                if self.process_monitoring_table.item(0, 2):
                    self.process_monitoring_table.item(0, 2).setText(
                        str(widget_info.get("pid", "N/A"))
                    )
                if self.process_monitoring_table.item(0, 3):
                    self.process_monitoring_table.item(0, 3).setText(
                        str(widget_info.get("hwnd", "N/A"))
                    )

                # Update other components based on app state
                if self.process_monitoring_table.item(1, 1):
                    self.process_monitoring_table.item(1, 1).setText(
                        "Running" if hasattr(self.app, "window_manager") else "N/A"
                    )
                if self.process_monitoring_table.item(2, 1):
                    self.process_monitoring_table.item(2, 1).setText(
                        "Running" if hasattr(self.app, "debug_console") else "N/A"
                    )
                if self.process_monitoring_table.item(3, 1):
    ...
    def _update_coordinates_monitoring(self):
        ...
        try:
            ...
            if widget_info:
                ...
                self.coordinates_table.item(0, 3).setText(
            ...
            if playable_info:
                self.coordinates_table.item(1, 1).setText(
                    str(playable_info.get("x", "N/A"))
                )
                self.coordinates_table.item(1, 2).setText(
                    str(playable_info.get("y", "N/A"))
                )
                self.coordinates_table.item(1, 3).setText(
                    f"{playable_info.get('width', 0)}x{playable_info.get('height', 0)}"
                )
            ...
            if hasattr(self.app, "overlay_window") and self.app.overlay_window:
                ...
                self.coordinates_table.item(2, 3).setText(
                    f"{overlay_rect.width()}x{overlay_rect.height()}"
                )
    ...
    def _update_mouse_tracking(self):
        ...
        try:
            ...
            self.mouse_tracking_table.item(0, 1).setText(
                self.current_mouse_data.get("current_position", "N/A")
            )
            ...
            if self.window_manager and self.mouse_tracker:
                ...
                if coords:
                    ...
                    widget_coords = coords.get("widget_window", {}).get(
                        "coordinates", {}
                    )
                    if widget_coords.get("width", 0) > 0:
                        if (
                            ...
                            self.mouse_tracking_table.item(1, 1).setText(
                    ...
                    if playable_coords.get("width", 0) > 0:
                        if (
                            ...
                            self.mouse_tracking_table.item(2, 1).setText(
                                f"{percent_x:.1f}%, {percent_y:.1f}% (BG:{bg_x},{bg_y})"
                            )
                        else:
