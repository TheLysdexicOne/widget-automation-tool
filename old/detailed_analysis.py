...
def analyze_for_text_regions(image_path, output_dir="analysis_output"):
    ...
    try:
        ...
        draw.rectangle([
            playable['x'], playable['y'],
            playable['x'] + playable['width'], playable['y'] + playable['height']
        ], outline='red', width=3)
        ...
        print(f"   Bright pixels (potential text): {len(bright_pixels)}/{len(pixels)} ({len(bright_pixels)/len(pixels)*100:.1f}%)")
        ...
        regions = {
            ...
            'center_content': (pw//8, ph//3, 7*pw//8, 2*ph//3),  # Main content
        ...
        for region_name, (x1, y1, x2, y2) in regions.items():
            ...
            print(f"   {region_name}: {bright_percent:.1f}% bright, {very_bright_percent:.1f}% very bright")
        ...
        overlay_path = os.path.join(output_dir, f"analysis_{os.path.basename(image_path)}")
...
def main():
    ...
    if results:
        ...
        for region_name in region_names:
            avg_bright = sum(r['text_regions'][region_name]['bright_percent'] for r in results) / len(results)
            avg_very_bright = sum(r['text_regions'][region_name]['very_bright_percent'] for r in results) / len(results)
            
            if avg_very_bright > 1.0:  # Good text candidate
                print(f"   ✅ {region_name}: {avg_bright:.1f}% bright, {avg_very_bright:.1f}% very bright - Good for text detection")
            elif avg_bright > 5.0:  # Potential text area
                print(f"   ⚠️  {region_name}: {avg_bright:.1f}% bright, {avg_very_bright:.1f}% very bright - Potential text area")
            else:
                print(f"   ❌ {region_name}: {avg_bright:.1f}% bright, {avg_very_bright:.1f}% very bright - Low text probability")
